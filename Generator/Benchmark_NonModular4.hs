...
module Benchmark_NonModular4 where
  ...
  arbitrary = getSize >>= \n -> mToMaybe 10000 n $ gen (n, -1)
    where gen :: Seed -> M Expr
          ...
          genAdd (n, max) = do n' <- mChoose (0, n - 1)
                               eA <- gen (n', max)
                               eB <- gen (n - 1, max)
                               mElements [Add eA eB, Add eB eA]
          genMul :: Seed -> M Expr
          genMul   (n, max) = do n' <- mChoose (0, n - 1)
                                 eA <- gen (n', max)
                                 eB <- gen (n - 1, max)
                                 mElements [Mul eA eB, Mul eB eA]
          genIf :: Seed -> M Expr
          genIf    (n, max) = do nA <- mChoose (0, n - 1)
                                 ...
                                 eB <- gen (nB, max)
                                 eC <- gen (n - 1, max)
                                 mElements [If eC eA eB, If eA eC eB, If eA eB eC]
          genEqual :: Seed -> M Expr
          genEqual (n, max) = do n' <- mChoose (0, n - 1)
                                 eA <- gen (n', max)
                                 eB <- gen (n - 1, max)
                                 mElements [Equal eA eB, Equal eB eA]
          genLam :: Seed -> M Expr
