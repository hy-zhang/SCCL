...
module Benchmark_NonModular2 where
  ...
  arbitrary = getSize >>= \n -> gen (n, -1)
    where gen :: Seed -> Gen Expr
          ...
          genAdd (n, max) = do n' <- choose (0, n - 1)
                               eA <- gen (n', max)
                               eB <- gen (n - 1, max)
                               elements [Add eA eB, Add eB eA]
          genMul :: Seed -> Gen Expr
          genMul   (n, max) = do n' <- choose (0, n - 1)
                                 eA <- gen (n', max)
                                 eB <- gen (n - 1, max)
                                 elements [Mul eA eB, Mul eB eA]
          genIf :: Seed -> Gen Expr
          genIf    (n, max) = do nA <- choose (0, n - 1)
                                 ...
                                 eB <- gen (nB, max)
                                 eC <- gen (n - 1, max)
                                 elements [If eC eA eB, If eA eC eB, If eA eB eC]
          genEqual :: Seed -> Gen Expr
          genEqual (n, max) = do n' <- choose (0, n - 1)
                                 eA <- gen (n', max)
                                 eB <- gen (n - 1, max)
                                 elements [Equal eA eB, Equal eB eA]
